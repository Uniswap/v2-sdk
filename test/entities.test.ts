    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      beforeAll(() => {
        tokens = ADDRESSES.map(
          (address, i) => new Token(CHAIN_ID, address, decimals[i])
        )
      })
      ...
      describe('Trade', () => {
        ...
        it('TradeType.EXACT_INPUT', () => {
          ...
          const expectedOutputAmount = CurrencyAmount.fromRawAmount(
            WETH9,
            '1662497915624478906'
          )
          ...
          expect(trade.executionPrice.toSignificant(18)).toEqual(
            '1.66249791562447891'
          )
          expect(trade.executionPrice.invert().toSignificant(18)).toEqual(
            '0.601504513540621866'
          )
          expect(trade.executionPrice.quote(inputAmount).quotient).toEqual(
            expectedOutputAmount.quotient
          )
          expect(
            trade.executionPrice.invert().quote(expectedOutputAmount).quotient
          ).toEqual(inputAmount.quotient)
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          ...
          const expectedInputAmount = CurrencyAmount.fromRawAmount(
            tokens[1],
            decimalize(1, tokens[1].decimals)
          )
          ...
          expect(
            trade.executionPrice.quote(expectedInputAmount).quotient
          ).toEqual(outputAmount.quotient)
          expect(
            trade.executionPrice.invert().quote(outputAmount).quotient
          ).toEqual(expectedInputAmount.quotient)
