...
describe('Trade', () => {
  ...
  const token2 = new Token(ChainId.EXPANSE, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(ChainId.EXPANSE, '0x0000000000000000000000000000000000000004', 18, 't3')
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WETH[ChainId.EXPANSE], JSBI.BigInt(1000)),
  ...
  it('can be constructed with EXPANSE as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], EXPANSE),
      CurrencyAmount.ether(JSBI.BigInt(100), EXPANSE),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(EXPANSE)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with EXPANSE as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], EXPANSE, token0),
  ...
  it('can be constructed with EXPANSE as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, EXPANSE),
      CurrencyAmount.ether(JSBI.BigInt(100), EXPANSE),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(EXPANSE)
  })
  it('can be constructed with EXPANSE as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, EXPANSE),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for EXPANSE currency input', () => {
      const result = Trade.bestTradeExactIn(
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(EXPANSE)
      expect(result[0].route.path).toEqual([WETH[ChainId.EXPANSE], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(EXPANSE)
      expect(result[1].route.path).toEqual([WETH[ChainId.EXPANSE], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })
    it('works for EXPANSE currency output', () => {
      const result = Trade.bestTradeExactIn(
        [pair_weth_0, pair_0_1, pair_0_3, pair_1_3],
      ...
      expect(result[0].route.path).toEqual([token3, token0, WETH[ChainId.EXPANSE]])
      expect(result[0].outputAmount.currency).toEqual(EXPANSE)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH[ChainId.EXPANSE]])
      expect(result[1].outputAmount.currency).toEqual(EXPANSE)
    })
  })
