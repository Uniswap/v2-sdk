...
describe('CurrencyAmount', () => {
  ...
  describe('#quotient', () => {
    it('returns the amount after multiplication', () => {
      const token = new Token(1, ADDRESS_ONE, 18)
      const amount = new CurrencyAmount(token, 100).multiply(new Percent(15, 100))
  ...
  describe('#ether', () => {
    it('produces ether amount', () => {
      const amount = new CurrencyAmount(Ether.onChain(1), 100)
      expect(amount.quotient).toEqual(JSBI.BigInt(100))
  ...
  it('token amount can be max uint256', () => {
    const amount = new CurrencyAmount(new Token(1, ADDRESS_ONE, 18), MaxUint256)
    expect(amount.quotient).toEqual(MaxUint256)
  })
  it('token amount cannot exceed max uint256', () => {
    expect(() =>
      new CurrencyAmount(new Token(1, ADDRESS_ONE, 18), JSBI.add(MaxUint256, JSBI.BigInt(1)))
    ).toThrow('AMOUNT')
  })
  ...
  describe('#toExact', () => {
    ...
    it('is correct for 18 decimals', () => {
      const token = new Token(1, ADDRESS_ONE, 18)
      const amount = new CurrencyAmount(token, 123e13)
