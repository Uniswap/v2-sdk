...
function resolveOptions(options) {
  ...
  const targets = (0, _helperCompilationTargets.default)(targetsOption, {
    ignoreBrowserslistConfig,
    configPath
  });
  return {
...
function instantiateProvider(factory, options, missingDependencies, dirname, debugLog, babelApi) {
  ...
  const getUtils = (0, _utils.createUtilsGetter)(new _importsCache.default()); // eslint-disable-next-line prefer-const
  ...
  const api = {
    ...
    shouldInjectPolyfill(name) {
      ...
      if (!polyfillsNames.has(name)) {
        throw new Error(`Internal error in the ${provider.name} provider: ` + `unknown polyfill "${name}".`);
      }
